        ...
        for boat in boats:
            if boat['mmsi'] == full_data.iloc[index][0]['device']['mmsi']:
                ...
                if(location['port'] and (len(boat['trips']) == 0 or boat['trips'][-1]['arrival_time'] != None)):
                    ...
                    boat['trips'].append({
                        'departure_time': datetime.fromisoformat(full_data.iloc[index][0]['navigation']['time']).timestamp(),
                ...
                if(location['port'] and location['destination']['name'] != boat['trips'][-1]['departure']['name']):
                   ...
                   boat['trips'][-1]['arrival_time'] = datetime.fromisoformat(full_data.iloc[index][0]['navigation']['time']).timestamp()
